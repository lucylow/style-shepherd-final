  ...
  const handlePredict = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.warning("Using mock predictions (API unavailable)");
      } else if (data && data.success && data.predictions) {
        setPredictions(data.predictions);
        toast.success(`Predicted sizes for ${data.predictions.length} brands`);
      } else {
        // Fall back to mock data
        const results = selectedBrands.map(brandId => {
          const brand = MOCK_BRANDS.find(b => b.id === brandId);
        ...
        setPredictions(results);
        toast.warning("Using mock predictions");
      }
    } catch (error) {
      ...
      setPredictions(results);
      toast.error("Failed to predict sizes, using mock data");
    } finally {
      setLoading(false);
      setShowResults(true);
    }
...
const SizePredictionCard = ({ prediction, index }: any) => {
  const brandName = prediction.brand_adjustments_applied?.brand || "Unknown";
  const primarySize = prediction.predicted_sizes?.primary || "M";
  const confidence = prediction.predicted_sizes?.confidence || 0.85;
  const alternatives = prediction.predicted_sizes?.alternatives || [];
  const returnRisk = prediction.fit_recommendation?.return_risk || 0.2;
  const normalized = prediction.normalized_measurements || {};
  const adjustmentNote = prediction.brand_adjustments_applied?.adjustment_note || "True to size";
  const explanation = prediction.explanation || `Size ${primarySize} recommended for ${brandName}`;
    ...
    >
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2 text-sm bg-rose-100 px-3 py-1 rounded-full">
          <span className={`px-3 py-1 rounded-full text-xs font-bold ${
            ...
            {returnRisk > 0.3 ? "⚠️ Medium Risk" : "✅ Low Risk"}
          </span>
      ...
      {alternatives.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          {alternatives.map((size: string, i: number) => (
      ...
      <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 p-4 rounded-xl border border-emerald-200">
        <div className="font-semibold text-emerald-800 mb-1">
          {prediction.fit_recommendation?.recommended_fit?.replace('_', ' ').toUpperCase() || "TRUE TO SIZE"}
        </div>
