...
async function checkVultr() {
  ...
  try {
    ...
    const res = await fetch(VULTR_URL, { method: 'HEAD', timeout: 3000 });
...
async function checkEleven() {
  ...
  try {
    ...
    const res = await fetch(url, { method: 'GET', headers: { 'xi-api-key': ELEVEN_KEY }, timeout: 3000 });
    return { ok: res.ok, status: res.status };
  } catch (err) {
    return { ok: false, error: String(err) };
  }
...
module.exports = { runServiceChecks, checkVultr, checkEleven, checkRaindrop };
